...
def input_validators() -> dict[str, Draft7Validator]:
    return {
...
def processed_validator() -> Draft7Validator:
    return Draft7Validator(load_schema("processed_order_event.json"), format_checker=FormatChecker())
...
def validate_processed_event(event: dict, schema_validator: Draft7Validator) -> None:
    errors = sorted(schema_validator.iter_errors(event), key=lambda item: item.path)
    if errors:
        raise ValueError("; ".join(error.message for error in errors))
...
def process_with_retry(
    event: dict,
    producer: Producer,
    output_schema_validator: Draft7Validator,
) -> None:
    ...
    try:
        while True:
            ...
            try:
                ...
                process_with_retry(event, producer, output_schema_validator)
