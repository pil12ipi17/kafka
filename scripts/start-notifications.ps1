...
try {
    ...
    docker compose --env-file env_files/config.env -f docker-compose.yml -f $notificationsCompose up -d --build notification-preferences-service notification-classifier-service notification-filter-service notification-router-service notification-digest-service
}
finally {
    Pop-Location
}
